...
% if SFCR show option of AutoRef
QSMSolverDict = cellstr(get(handles.VarQSMSolver, 'String'));
if handles.Params.R2starFlag == 1 && strcmpi(QSMSolverDict(get(handles.VarQSMSolver, 'Value')), 'SFCR')
...
if ~strcmpi(contents{get(hObject,'Value')}, 'SFCR')
    set([handles.checkboxAutoRef], 'Visible', 'Off')
else
