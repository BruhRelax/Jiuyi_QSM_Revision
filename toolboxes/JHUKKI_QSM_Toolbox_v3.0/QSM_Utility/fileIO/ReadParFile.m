...
while (~feof(fid))
    ...
    elseif (tline(1) == '.')    % basic parameter part
        ...
        if(findstr('number of slices', tline) > 0 )
            aa = tline(findstr(':', tline)+1:end);
            aa = str2num(aa);
            header.nslices = aa(1);
        end
        
        %  Look for the FOV (ap, fh, rl)
        if(findstr('FOV (', tline) > 0 )
            aa = tline(findstr(':', tline)+1:end);
            header.fov = str2num(aa);
        end
        
        %  Look for the patient position
        if(findstr('Patient position', tline) > 0 )
            aa = tline(findstr(':', tline)+1:end);
            aa = aa(4:end);  
            aaind = strfind(aa, ' ');
        ...
        if(findstr('Angulation midslice(', tline) > 0 )
            aa = tline(findstr(':', tline)+1:end);
            header.ang = str2num(aa);
        end
        
        %  Look for number of echos
        if(findstr('number of echoes', tline) > 0 )
            aa = tline(findstr(':', tline)+1:end);
            aa = str2num(aa);
            header.nEchoes = aa(1);
        end
        ...
        if(findstr('number of dynamics', tline) > 0 )
            aa = tline(findstr(':', tline)+1:end);
            aa = str2num(aa);
        ...
        %  Look for TR
        if(findstr('Repetition time [', tline) > 0 )
            aa = tline(findstr(':', tline)+1:end);
            aa = str2num(aa);
            header.TR = aa(1);
        end
...
Tpo = []; Tpp = [];
if strcmpi(header.patient_orientation,'supine')
    Tpo = [1,0,0;0,1,0;0,0,1];
    rev_Tpo = [1,0,0;0,1,0;0,0,1];
elseif strcmpi(header.patient_orientation,'prone')
    Tpo = [-1,0,0;0,-1,0;0,0,1];
    rev_Tpo = [-1,0,0;0,-1,0;0,0,1];  
